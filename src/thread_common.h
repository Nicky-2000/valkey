...
// Assuming CACHE_LINE_SIZE is defined elsewhere (e.g., in a central config file or server.h).
// If not, you might define a common default here, like #define CACHE_LINE_SIZE 64.
// For the example below, I'm using 64 directly as was in your JobQueue struct.
 ...
 * This variable is 'extern' here, meaning its definition will be in
 * exactly one .c file (e.g., thread_common.c). */
...
/* --- Generic Job Queue Definitions --- */

typedef void (*job_handler)(void *);

/* Threads receive a function pointer to call using the data provided */
typedef struct job {
    job_handler handler;
    void *data;
} job;
...
// Function prototypes for the generic JobQueue operations
