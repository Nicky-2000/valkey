...
static void *workerThreadEntryPoint(void *arg) {
    ...
    threadSafeQueue *task_queue = pool->task_queue;
    task *t;
...
threadPool *threadPoolCreate(int num_threads) {
    ...
    pool->task_queue = zmalloc(sizeof(threadSafeQueue));
    if (!pool->task_queue) {
        zfree(pool);
        return NULL;
    }
...
void threadPoolDestroy(threadPool *pool) {
    ...
    // Step 2: Wait for any remaining tasks to finish
    threadPoolWaitAll(pool);

    // Step 3: Signal the queue to shut down, waking all workers
