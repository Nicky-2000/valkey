...
/* Define the thread-local integer to store each thread's unique ID.
 * This is the *single definition* for the 'thread_id' declared extern in thread_common.h. */
...
/* --- Generic Job Queue Implementations --- Used to send jobs from the main-thread to the worker thread (IO Thread or RDB Thread). */
